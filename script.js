...
function renderEngineDropdown() { engineList.innerHTML = ''; enginesData.forEach(eng => { const li = document.createElement('li'); li.innerHTML = `<i class="${eng.icon}"></i><span>${eng.name}</span>`; li.onclick = () => { setSearchEngine(eng.id); engineList.classList.remove('show'); }; engineList.appendChild(li); }); }
